...
// COMBAT SIMULATION HANDLER SUBROUTINE
void executeCombatScenario(Player& playerInstance, const CityLevel& targetCity) {
    ...
    while (activeZombieHp > 0 && playerInstance.getHp() > 0) {
        std::cout << " Combat Stance -> Ace HP: " << activeZombieHp << " | Your HP: " << playerInstance.getHp() << "% | Medkits: " << playerInstance.getMedkits() << "\n";
        std::cout << " 1. Shoot with Assault Rifle (-1 Ammo)\n 2. Slash with Machete\n 3. Use Field Trauma Medkit (-1 Kit)\n Choose stance (1-3): ";
        int fightChoice = getValidatedInput(1, 3);
        ...
        if (fightChoice == 1) {
            ...
            if (playerInstance.getMedkits() > 0) {
                playerInstance.modifyMedkits(-1);
                playerInstance.modifyHp(45);
                std::cout << "\n[+] Combat Patch-up! Administered field trauma treatment and restored 45% HP.\n";
            }
            else {
                std::cout << "\n[X] MEDICAL FAILURE: No trauma kits remain in your pack! Turn wasted.\n";
            }
        }
...
int main() {
    ...
    bool completeApplicationRunning = true;

    // MASTER SESSION REPLAY LOOP
    while (completeApplicationRunning) {
        ...
        while (systemLobbyRunning && player->getHp() > 0) {
            ...
            if (lobbyChoice == 1) {
                ...
                while (playingFieldActive && player->getHp() > 0) {
                    ...
                    if (gameAction == 1) {
                        ...
                        if (player->getMachetes() >= activeCity.getRequiredMachetes() &&
                            ...
                            narrator.printCityVictory(activeCity); playingFieldActive = false;
                            if (activeLevelIndex == player->getHighestLevel()) { player->setHighestLevel(activeLevelIndex + 1); }
                        }
                        else {
                            std::cout << "\n=========================================================================\n"; std::cout << "[X] EXTRACTION DENIED: The checkpoint perimeter line out of " << activeCity.getName() << " is completely locked down!\n";
                            if (activeCity.getName() == "Austin") { std::cout << "Reason: Heavy high-security chain blockades encircle the border checkpoint gates.\n";
                            std::cout << "You cannot push past without clear tools to slash down guards and cut thick steel bounds!\n"; }
                            else if (activeCity.getName() == "Houston") { std::cout << "Reason: Long-range acid variants have trapped the marshaling docks.\n"; std::cout << "You must secure firepower and structural clearance tools to survive the push through!\n"; }
                ...
                }if (player->getHp() <= 0) { narrator.printGameOverScreen(false, player->getName()); systemLobbyRunning = false; }
                else if (player->getHighestLevel() >= static_cast<int>(levels.size())) { narrator.printGameOverScreen(true, player->getName());
                systemLobbyRunning = false; }
            }// LOBBY OPTION 2: INSPECT LOCKER INVENTORY// =========================================================================
            ...
            //========================================// LOBBY OPTION 3: REGIONAL SELECT MATRIX// =======================================
            else if (lobbyChoice == 3) {narrator.printLevelSelectionMenu(levels, player->getHighestLevel());int selectTarget = getValidatedInput(0, static_cast<int>(levels.size()));
            ...
            while (simulationActive && player->getHp() > 0) { CityLevel& simCity = levels[selectedLevelIdx]; 
            if (triggerIntro) { narrator.printCityIntro(simCity); 
            triggerIntro = false; }narrator.printPlayerDashboard(*player, simCity); 
            std::cout << "Simulation Options:\n1. Search simulation blocks (+3 Ammo)\n2. Rest coordinates (+20% HP)\n3. Exit simulation\nChoose Action (1-3): "; 
            int simAct = getValidatedInput(1, 3); 
            if (simAct == 1) { std::cout << "\nSearching simulation blocks...\n"; 
            player->modifyAmmo(3); std::cout << "[+] Found safe container reserves. Loaded +3 ammo rounds.\n"; }
            else if (simAct == 2) { player->modifyHp(20); std::cout << "\n[+] Scoped resting coordinates. Restored 20% health.\n"; } 
            else if (simAct == 3) { std::cout << "\n[✓] Simulation sector path exited cleanly.\n"; simulationActive = false; } }
            if (player->getHp() <= 0) { narrator.printGameOverScreen(false, player->getName()); systemLobbyRunning = false; }
            }
            ...
            // LOBBY OPTION 4: LOAD FROM STORAGE//
            else if (lobbyChoice == 4) {
                loadGameProgress(*player);
            }
                // LOBBY OPTION 5: SAVE TO STORAGE//
            else if (lobbyChoice == 5) {
                saveGameProgress(*player); }
                // LOBBY OPTION 6: SECURE CLOSURE//
            else if (lobbyChoice == 6) { systemLobbyRunning = false; completeApplicationRunning = false;
            std::cout << "\nClosing security uplink interface lines...\n"; }}
            // POST-RUN LOUNGE SYSTEM REPLAY SOLICITATION
            if (player->getHp() <= 0 || player->getHighestLevel() >= static_cast<int>(levels.size())) 
            {std::cout << "\n---------------------------------------------------------\n";
            std::cout << " Do you want to play again from the beginning?\n";
            std::cout << " 1. Yes, spin up a new survival run character session\n";
            std::cout << " 2. No, shut down the console terminal application\n";
            std::cout << " Choose option (1-2): ";
            int playAgainChoice = getValidatedInput(1, 2);
            if (playAgainChoice == 2) {
                completeApplicationRunning = false;
            }
            }
}
